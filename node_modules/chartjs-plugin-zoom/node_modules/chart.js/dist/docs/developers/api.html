...
</code></pre> <blockquote> <p><strong>Note:</strong> replacing the data reference (e.g. <code>myLineChart.data = {datasets: [...]}</code> only works starting <strong>version 2.6</strong>. Prior that, replacing the entire data object could be achieved with the following workaround: <code>myLineChart.config.data = {datasets: [...]}</code>.</p> </blockquote> <p>A <code>config</code> object can be provided with additional configuration for the update process. This is useful when <code>update</code> is manually called inside an event handler and some different animation is desired.</p> <p>The following properties are supported:</p> <ul> <li><strong>duration</strong> (number): Time for the animation of the redraw in milliseconds</li> <li><strong>lazy</strong> (boolean): If true, the animation can be interrupted by other animations</li> <li><strong>easing</strong> (string): The animation easing function. See <a href="../configuration/animations.html">Animation Easing</a> for possible values.</li> </ul> <p>Example:</p> <pre><code class="lang-javascript">myChart.update({
    duration: <span class="hljs-number">800</span>,
    easing: <span class="hljs-string">&apos;easeOutBounce&apos;</span>
})
</code></pre> <p>See <a href="updates.html">Updating Charts</a> for more details.</p> <h2 id="reset">.reset()</h2> <p>Reset the chart to it&apos;s state before the initial animation. A new animation can then be triggered using <code>update</code>.</p> <pre><code class="lang-javascript">myLineChart.reset();
</code></pre> <h2 id="renderconfig">.render(config)</h2> <p>Triggers a redraw of all chart elements. Note, this does not update elements for new data. Use <code>.update()</code> in that case.</p> <p>See <code>.update(config)</code> for more details on the config object.</p> <pre><code class="lang-javascript"><span class="hljs-comment">// duration is the time for the animation of the redraw in milliseconds</span>
<span class="hljs-comment">// lazy is a boolean. if true, the animation can be interrupted by other animations</span>
myLineChart.render({
    duration: <span class="hljs-number">800</span>,
...
</code></pre> <h2 id="stop">.stop()</h2> <p>Use this to stop any current animation loop. This will pause the chart during any current animation frame. Call <code>.render()</code> to re-animate.</p> <pre><code class="lang-javascript"><span class="hljs-comment">// Stops the charts animation loop at its current frame</span>
myLineChart.stop();
<span class="hljs-comment">// =&gt; returns &apos;this&apos; for chainability</span>
</code></pre> <h2 id="resize">.resize()</h2> <p>Use this to manually resize the canvas element. This is run each time the canvas container is resized, but you can call this method manually if you change the size of the canvas nodes container element.</p> <pre><code class="lang-javascript"><span class="hljs-comment">// Resizes &amp; redraws to fill its container element</span>
myLineChart.resize();
<span class="hljs-comment">// =&gt; returns &apos;this&apos; for chainability</span>
</code></pre> <h2 id="clear">.clear()</h2> <p>Will clear the chart canvas. Used extensively internally between animation frames, but you might find it useful.</p> <pre><code class="lang-javascript"><span class="hljs-comment">// Will clear the canvas that myLineChart is drawn on</span>
myLineChart.clear();
<span class="hljs-comment">// =&gt; returns &apos;this&apos; for chainability</span>
</code></pre> <h2 id="tobase64image">.toBase64Image()</h2> <p>This returns a base 64 encoded string of the chart in it&apos;s current state.</p> <pre><code class="lang-javascript">myLineChart.toBase64Image();
<span class="hljs-comment">// =&gt; returns png data url of the image on the canvas</span>
</code></pre> <h2 id="generatelegend">.generateLegend()</h2> <p>Returns an HTML string of a legend for that chart. The legend is generated from the <code>legendCallback</code> in the options.</p> <pre><code class="lang-javascript">myLineChart.generateLegend();
<span class="hljs-comment">// =&gt; returns HTML string of a legend for this chart</span>
</code></pre> <h2 id="getelementatevente">.getElementAtEvent(e)</h2> <p>Calling <code>getElementAtEvent(event)</code> on your Chart instance passing an argument of an event, or jQuery event, will return the single element at the event position. If there are multiple items within range, only the first is returned. The value returned from this method is an array with a single parameter. An array is used to keep a consistent API between the <code>get*AtEvent</code> methods.</p> <pre><code class="lang-javascript">myLineChart.getElementAtEvent(e);
<span class="hljs-comment">// =&gt; returns the first element at the event point.</span>
</code></pre> <p>To get an item that was clicked on, <code>getElementAtEvent</code> can be used.</p> <pre><code class="lang-javascript"><span class="hljs-function"><span class="hljs-keyword">function</span> <span class="hljs-title">clickHandler</span>(<span class="hljs-params">evt</span>) </span>{
...
</code></pre> <h2 id="getelementsatevente">.getElementsAtEvent(e)</h2> <p>Looks for the element under the event point, then returns all elements at the same data index. This is used internally for &apos;label&apos; mode highlighting.</p> <p>Calling <code>getElementsAtEvent(event)</code> on your Chart instance passing an argument of an event, or jQuery event, will return the point elements that are at that the same position of that event.</p> <pre><code class="lang-javascript">canvas.onclick = <span class="hljs-function"><span class="hljs-keyword">function</span>(<span class="hljs-params">evt</span>)</span>{
...
</code></pre> <p>This functionality may be useful for implementing DOM based tooltips, or triggering custom behaviour in your application.</p> <h2 id="getdatasetatevente">.getDatasetAtEvent(e)</h2> <p>Looks for the element under the event point, then returns all elements from that dataset. This is used internally for &apos;dataset&apos; mode highlighting</p> <pre><code class="lang-javascript">myLineChart.getDatasetAtEvent(e);
<span class="hljs-comment">// =&gt; returns an array of elements</span>
</code></pre> <h2 id="getdatasetmetaindex">.getDatasetMeta(index)</h2> <p>Looks for the dataset that matches the current index and returns that metadata. This returned data has all of the metadata that is used to construct the chart.</p> <p>The <code>data</code> property of the metadata will contain information about each point, rectangle, etc. depending on the chart type.</p> <p>Extensive examples of usage are available in the <a href="https://github.com/chartjs/Chart.js/tree/master/test" target="_blank">Chart.js tests</a>.</p> <pre><code class="lang-javascript"><span class="hljs-keyword">var</span> meta = myChart.getDatasetMeta(<span class="hljs-number">0</span>);
<span class="hljs-keyword">var</span> x = meta.data[<span class="hljs-number">0</span>]._model.x
</code></pre> </section> </div> <div class="search-results"> <div class="has-results"> <h1 class="search-results-title"><span class="search-results-count"></span> results matching "<span class="search-query"></span>"</h1> <ul class="search-results-list"></ul> </div> <div class="no-results"> <h1 class="search-results-title">No results matching "<span class="search-query"></span>"</h1> </div> </div> </div> </div> </div> </div> <a href="./" class="navigation navigation-prev" aria-label="Previous page: Developers"> <i class="fa fa-angle-left"></i> </a> <a href="updates.html" class="navigation navigation-next" aria-label="Next page: Updating Charts"> <i class="fa fa-angle-right"></i> </a> </div> <script>var gitbook = gitbook || [];
